...
class ChatController extends GetxController {
  ...
  Future<List<QueryDocumentSnapshot>> getGroups() async {
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        var unJoinedGroups = snapshot.docs.where((doc) => ((doc.data() as Map).containsKey("members") && !(doc['members'] as List<dynamic>).contains('${uid}_${currentUserData.value.getValueOfKey("name") ?? ""}'))).toList();
        var joinedGroups = snapshot.docs.where((doc) => ((doc.data() as Map).containsKey("members") && (doc['members'] as List<dynamic>).contains('${uid}_${currentUserData.value.getValueOfKey("name") ?? ""}'))).toList();
  ...
  getGroupsStream() async {
    try {
      chatService.groupCollection.orderBy("createdAt").snapshots().listen((snapshot) {
        if (snapshot.docs.isNotEmpty) {
  ...
  Future<dynamic> joinExitGroup({
    ...
    required int amount,
      ...
      dialog.show();
    try {
      var value = await walletContoller.joinGroupEvent(amount, "Join Group Event - $groupName");
      if(value!=null){

        var data = await chatService.toggleGroupJoin(groupId: groupId, userName: userName, groupName: groupName);
        if(data!=null){
          var snapshot = await data.parent.where("groupId",isEqualTo: data.id).get();
          if (snapshot.docs.isNotEmpty) {
  ...
  Future<Map?> createGroup({
    ...
    try {
      ...
      var value = await walletContoller.groupCreation(groupFee.value ?? 0, "Group-creation");
      if(value!=null){
        String? url;
  ...
  Future<bool?> joinedOrNot(String userName, String groupId, String groupName) async {
  ...
  Future uploadChatImage(
    ...
    dialog.show();
    try {
      var reference = _storage.ref().child("${prefix}_${groupId}_${FileUtils.getFileDisplayName(image)}");
  ...
  Future uploadImage(
    ...
    dialog.show();
    try {
      var reference = _storage.ref().child("${prefix}_${FileUtils.getFileDisplayName(image)}");
      UploadTask uploadTask = reference.putFile(image);
      TaskSnapshot taskSnapshot = await uploadTask.whenComplete(() => null);
      var downloadURL = await taskSnapshot.ref.getDownloadURL();
      dialog.dismiss();
  ...
  Future approvedRequest({
    ...
    try {
      await chatService.approvedRequest(
          ...
          groupName: (currentGroup.value?.data() as Map).getValueOfKey("groupName") ?? "",
          userName: name,
          requestId: requestId,
          userId: userId);
  ...
  double? getGroupMember({Map? groupData}) {
    ...
    if(groupData?.containsKey("members") ?? false){
      try{
        ...
        poolAmount = (totalMembers-1) * double.parse(groupData.getValueOfKey("joinAmount").toString());
  ...
  Future<bool> checkMemberAnswered(String collectionName, String documentId,String userId) async {
    try {
      ...
      List<Map<String, dynamic>> userOutcome = List.from(currentData['userOutcome'] ?? []);
      bool userIdExists = userOutcome.any((element) => element['userId'] == userId);
      return userIdExists;
    } catch (error) {
  ...
  Future<void> addOutcomeToGroup(String collectionName, String documentId, Map<String, dynamic> newData, String userId) async {
    try {
      DocumentReference documentRef = FirebaseFirestore.instance.collection(collectionName).doc(documentId);
      DocumentSnapshot snapshot = await documentRef.get();
      Map<String, dynamic> currentData = snapshot.data() as Map<String, dynamic>;
      ...
      if (documentSnapshot.exists) {
        ...
        if (data != null && data.containsKey('userOutcome')) {
          ...
          for(var outcome in data['userOutcome']){
            ...
            } else if (outcome['selectedOutCome'].toString().toLowerCase() == 'no') {
              noCount++;
            }
          }
  ...
  Future<bool> groupCreationEligibility({required WalletContoller controller}) async {
    try {
      ...
      if (data != null && data.containsKey("group_creation_fee")) {
        if (data.containsKey("group_creation_fee")) {
          ...
          if ((wallet.currentAmount + wallet.onlyInAppUseableAmount) >= (data.getValueOfKey("group_creation_fee") as num)) {
  ...
  Future<bool> groupEventJoinEligibilityCheck({required WalletContoller controller,required int groupJoinAmount}) async {
    try {
      var wallet = await controller.getWalletData(uid);
      if ((wallet.currentAmount + wallet.onlyInAppUseableAmount) >= groupJoinAmount) {
        return true;
      }
      return false;
    } catch (e) {
