...
class ChatService {
  ...
  Future<DocumentReference?> toggleGroupJoin(
    ...
    DocumentSnapshot documentSnapshot = await userDocumentReference.get();
    if (!(await userDocumentReference.doesKeyExist(docRef: userDocumentReference, key: "groups"))) {
      await userDocumentReference.set({"groups": []}, SetOptions(merge: true)); // merge: true ensures existing fields are not overwritten
    }
